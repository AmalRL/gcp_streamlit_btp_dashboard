...
# -----------------------------
# SSL FIX (important for Render/Linux)
# -----------------------------
...
intervention_df = load_csv_from_gcp("interventions_4weeks.csv")
...
# -----------------------------
intervention_df["created_at"] = pd.to_datetime(intervention_df["created_at"])

df2 = intervention_df[
    (intervention_df["created_at"] >= pd.to_datetime(selected_sunday)) &
    (intervention_df["created_at"] <= pd.to_datetime(selected_saturday))
]


if not df2.empty:

    agg_df = df2.groupby("campaign_type", as_index=False).agg(
        total_interventions=("total_interventions", "sum"),
        delivered_interventions=("delivered_interventions", "sum")
    )

    agg_df["delivery_percentage"] = (
        agg_df["delivered_interventions"] * 100 / agg_df["total_interventions"]
    ).round(2)
    ...
    st.warning("No intervention data found for selected date range")


st.caption("Source: Google Cloud Storage (Secure Credentials via Env Vars)")
